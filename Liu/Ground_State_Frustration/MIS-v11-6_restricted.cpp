...
struct Params { // 定义参数结构体 Params，用于封装程序运行所需的各种参数设置
    ...
    bool save_state; // 布尔标志，指示是否需要将最终状态保存到文件
...
void save_state_to_file_packed(const std::vector<uint64_t>& lattice_packed_array, // 函数 save_state_to_file_packed：将位打包的晶格状态和MIS状态保存到文本文件
    ...
    std::cout << "Saved combined state to " << filename << std::endl; // 向标准输出流打印保存成功的信息和文件名
...
void calculate_and_print_mis_statistics( // 函数 calculate_and_print_mis_statistics：计算并打印与MIS相关的各种统计数据
    ...
    std::cout << "\n--- Statistics ---" << std::endl; // 输出统计信息部分的标题
    std::cout << "  (d, L, W, rho, seed): (" << p.d << ", " << p.L << ", " << p.W << ", " << format_rho_string(p.rho) << ", " << p.seed << ")" << std::endl; // 输出当前模拟使用的参数
    ...
    std::cout << "  Number of nodes in active graph G (lattice == 1): " << num_nodes_in_active_graph << std::endl; // 输出活跃图G中的节点总数
    std::cout << "    Nodes in U partition of G: " << mis_result.nU << std::endl; // 输出G的U部分的节点数
    std::cout << "    Nodes in V partition of G: " << mis_result.nV << std::endl; // 输出G的V部分的节点数
    std::cout << "  Proportion of 1s in final lattice (after rules): " << std::fixed << std::setprecision(4) << proportion_ones_in_final_lattice << std::endl; // 输出最终晶格中1的比例 (保留4位小数)

    long long sum_mis = mis_result.mis_nodes_global_indices.size(); // MIS的大小 (即MIS中的节点总数)
    std::cout << "  Size of Maximum Matching (MCM): " << mis_result.mcm_size << std::endl; // 输出最大匹配 (MCM) 的大小
    std::cout << "  Size of Maximum Independent Set (MIS): " << sum_mis << std::endl; // 输出最大独立集 (MIS) 的大小
    ...
    if (num_nodes_in_active_graph > 0) { // 如果活跃图G非空
        ...
        std::cout << "  sum(MIS) / |V(G)|: " << std::fixed << std::setprecision(4) << mis_ratio_to_G_size << std::endl; // 输出该比率
    } else { // 如果活跃图G为空
        std::cout << "  sum(MIS) / |V(G)|: N/A (G is empty)" << std::endl; // 提示该比率不适用
    }

    if (order_parameter_is_valid) { // 如果序参数有效
        std::cout << "  MIS nodes count in U part (of G): " << mis_nodes_in_U_partition << std::endl; // 输出MIS在G的U部分的节点数
        std::cout << "  MIS nodes count in V part (of G): " << mis_nodes_in_V_partition << std::endl; // 输出MIS在G的V部分的节点数
        std::cout << "  Order Parameter ((MIS_U_nodes/|U_part_of_G|) - (MIS_V_nodes/|V_part_of_G|)): " << std::fixed << std::setprecision(4) << order_parameter << std::endl; // 输出计算得到的序参数
    } else { // 如果序参数无效 (因为G为空)
        std::cout << "  Order Parameter: N/A (G is empty)" << std::endl; // 提示序参数不适用
    }

    std::cout << "  Graph generation & rule application time: " << std::fixed << std::setprecision(2) << generation_duration.count() << " ms" << std::endl; // 输出图生成和规则应用的总耗时 (毫秒，保留2位小数)
    std::cout << "  MIS finding time (incl. G construction, Hopcroft-Karp): " << std::fixed << std::setprecision(2) << mis_duration.count() << " ms" << std::endl; // 输出MIS计算的总耗时 (毫秒，保留2位小数)
...
// g++ -O3 -march=native MIS-v11-6.cpp -o MIS;./MIS
...
int main(int argc, char* argv[]) { // 主函数：程序入口点
    ...
    if (argc > 5) p.save_state = (std::stoi(argv[5]) == 1); // 第5个参数：1表示保存状态，0表示不保存
    if (argc > 6) { // 第6个额外参数视为随机数种子
        try {
            long long temp_seed = std::stoll(argv[6]); // 尝试将参数字符串转换为long long类型
            ...
            std::cerr << "Warning: Seed value " << argv[6] << " is too large. Using random_device." << std::endl; // 警告
            p.seed = 0; // 使用随机设备
        } catch (const std::invalid_argument& ia) { // 捕获stoll转换时可能发生的无效参数异常
            std::cerr << "Warning: Invalid seed value '" << argv[6] << "'. Using random_device." << std::endl; // 警告
            p.seed = 0; // 使用随机设备
        }
    }
    if (argc > 7) p.periodic_boundary = (std::stoi(argv[7]) == 1); // 第7个参数：1表示使用周期性边界，0表示不使用

    std::mt19937 gen; // 创建一个梅森旋转算法的随机数生成器对象 (std::mt19937)
    if (p.seed == 0) { // 如果参数中的种子被设置为0 (表示应由随机设备生成)
        std::random_device rd; // 创建一个随机设备对象，用于获取高质量的随机种子
        unsigned int actual_seed = rd(); // 从随机设备生成一个实际种子
        gen.seed(actual_seed); // 使用这个实际种子来初始化随机数生成器
        std::cout << "Using random_device generated seed: " << actual_seed << " (input was 0)" << std::endl; // 打印所使用的实际种子
        p.seed = actual_seed; // 更新参数结构中的种子值为实际使用的种子
    } else { // 如果提供了非0的种子
        gen.seed(p.seed); // 直接使用参数中提供的种子来初始化随机数生成器
        std::cout << "Using provided seed: " << p.seed << std::endl; // 打印所使用的种子
    }
    ...
    try { // 使用try-catch块来处理计算总节点数时可能发生的错误 (如溢出)
        ...
        if (total_nodes_val < 0) total_nodes_val = 0; // 作为最终保障，如果 каким-то образом total_nodes_val 为负，则强制为0 (更佳做法是早些抛出错误)
    ...
    std::cout << "Parameters: d=" << p.d << ", L=" << p.L << ", W=" << p.W << ", rho=" << p.rho
              << ", seed=" << p.seed << ", periodic_boundary=" << p.periodic_boundary 
              << ", total_nodes=" << p.total_nodes_for_bitpacking;
    if(!p.periodic_boundary) //只在不使用周期性边界条件时考虑约束性外部边界条件
    {
        std::cout<<", boundary_condition="<< boundary_prefer;
    }
    ...
    if (p.total_nodes_for_bitpacking == 0) {
        ...
        MisResult empty_result; // 创建一个空的MIS结果对象
        empty_result.nU = 0; empty_result.nV = 0; empty_result.mcm_size = 0; // 初始化其成员
        calculate_and_print_mis_statistics(empty_result, p, {}, {}); // 打印空的统计数据 (耗时均为0)
        return 0; // 程序正常退出，返回0
    }
    
    if (p.save_state) { // 如果参数设置了保存状态
        std::string rho_str = format_rho_string(p.rho);
        std::string boundary_str = p.periodic_boundary? "-Periodic":"-NonPeriodic";
        if(!p.periodic_boundary){boundary_str = boundary_str +"-"+boundary_prefer;}
        dir = "MIS-graph/mis-data/d="+ std::to_string(p.d) + // 构建完整的文件名，包含维度、尺寸、rho和种子等参数信息
                           "-L=" + std::to_string(p.L) + "-W=" + std::to_string(p.W) + boundary_str  + 
                           "-rho=" + rho_str + "-seed=" + std::to_string(p.seed);
    }
    ...
    {
        // 对边界节点进行特定处理，例如根据奇偶性和类型'A'/'B'清零某些边界点
        // 这里的 'A' 可能代表一种特定的边界处理规则 (例如，清零坐标和为偶数的边界点)
        std::cout<< "Boundary condition: "<< boundary_prefer <<std::endl;
        lattice_packed =  process_boundary_nodes_ultra_fast( lattice_packed, p, boundary_prefer); 
    }
    ...
    if (p.save_state) { // 如果参数设置了保存状态
        save_state_to_file_packed(lattice_packed, mis_result.mis_lattice_representation_packed, p, dir); // 将最终的晶格状态和MIS状态保存到文件
    }

    calculate_and_print_mis_statistics(mis_result, p, generation_duration, mis_duration); // 计算并打印与MIS相关的各种统计数据及耗时信息
    std::string restricted_filename = dir+"/restricted_states.txt";

    auto time_start_flipping = std::chrono::high_resolution_clock::now();
    ...
    std::cout<< "Active spin size: "<< active_spin_list.size() << std::endl;
    ...
    auto time_end_flipping = std::chrono::high_resolution_clock::now();

    std::chrono::duration<double> flipping_time = time_end_flipping-time_start_flipping;
    std::cout <<"Time use for flipping process: " << std::setw(3) << flipping_time.count() << " second" << std::endl;

    std::string stats_restricted_filename = dir+"/restricted_stats.txt";
    std::string result_restricted_filename = dir + "/restricted_result.txt";
    ...
    result_file << "index, occupation state in ground state, change number, change energy" << std::endl;
    ...
    stats_file << "energy shift value:  " <<std::endl;
    ...
    stats_file << "Change vertex number: " << std::endl;
