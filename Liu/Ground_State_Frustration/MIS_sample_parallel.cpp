...
void save_state_to_file_packed(const std::vector<uint64_t>& lattice_packed_array, // 函数 save_state_to_file_packed：将位打包的晶格状态和MIS状态保存到文本文件   
                               const std::vector<uint64_t>& Largest_connected_component_packed, //位打包的晶格中最大联通分量
                               const std::vector<uint64_t>& mis_packed_array, // 位打包的MIS表示 (1表示节点属于MIS, 0则不属于)
                               const Params& p, // 程序参数，用于生成文件名和获取总节点数等信息
                                std::string dir = "MIS-graph/mis-data" //输出文件夹名
                                 ) { 
    
    std::string filename = dir + "/combined-state.txt";
    
    #if __has_include(<filesystem>) // 如果系统支持C++17的<filesystem>库
        if (!fs::exists(dir)) { // 检查目标目录是否存在
            fs::create_directories(dir); // 如果目录不存在，则递归创建它及其所有父目录
        }
    #else // 如果系统不支持<filesystem>，使用兼容的备用方案
        struct stat st; // 定义文件状态结构体
        if (stat(dir.c_str(), &st) == -1) { // 尝试获取目录状态，如果失败 (通常意味着目录不存在)
             create_directories_compat(dir); // 调用兼容的目录创建函数
        }
    #endif // __has_include(<filesystem>) 结束

    std::ofstream outfile(filename); // 创建并打开一个输出文件流，关联到指定文件名
    if (!outfile.is_open()) { // 如果文件未能成功打开
        std::cerr << "Error: Could not open file " << filename << " for writing." << std::endl; //向标准错误流输出错误信息
        return; // 提前返回，不执行后续写入操作
    }
    
    outfile << "Lattice:" << std::endl; // 在文件中写入 "Lattice:" 标签
    if (p.total_nodes_for_bitpacking > 0) { // 如果晶格中有节点
        outfile << get_bit(lattice_packed_array, 0); // 写入第一个节点的状态 (0或1)
        for (long long i = 1; i < p.total_nodes_for_bitpacking; ++i) { // 遍历剩余的所有节点
            outfile << " " << get_bit(lattice_packed_array, i); // 写入节点状态，用空格分隔
        }
    }
    outfile << std::endl; // 在晶格数据后换行
    
    outfile << " Largest connected component: " << std::endl;
    if (p.total_nodes_for_bitpacking > 0) { // 如果晶格中有节点
        outfile << get_bit(Largest_connected_component_packed, 0); // 写入第一个节点的状态 (0或1)
        for (long long i = 1; i < p.total_nodes_for_bitpacking; ++i) { // 遍历剩余的所有节点
            outfile << " " << get_bit(Largest_connected_component_packed, i); // 写入节点状态，用空格分隔
        }
    }
    outfile << std::endl; // 在最大联通分量数据后换行

    outfile << "MIS:" << std::endl; // 在文件中写入 "MIS:" 标签
    if (p.total_nodes_for_bitpacking > 0) { // 如果晶格中有节点 (MIS状态与晶格节点一一对应)
        outfile << get_bit(mis_packed_array, 0); // 写入第一个节点的MIS归属状态 (0或1)
        for (long long i = 1; i < p.total_nodes_for_bitpacking; ++i) { // 遍历剩余的所有节点
            outfile << " " << get_bit(mis_packed_array, i); // 写入节点的MIS归属状态，用空格分隔
        }
    }
    outfile << std::endl; // 在MIS数据后换行
    
    outfile.close(); // 关闭文件流，确保所有数据写入磁盘
} // save_state_to_file_packed 函数定义结束

void save_restricted_MIS_to_file_packed(const long long& restricted_index,// 函数 save_state_to_file_packed：将约束下得到的MIS状态保存到文本文件
                                        const std::vector<uint64_t>& restricted_mis_packed_array,
                                        const Params& p,
                                        std::string dir = "MIS-graph/mis-data"  )
{
    std::vector<int> restricted_spin_coords = index_to_coords(restricted_index, p.L, p.W, p.d);
    std::string filename = dir + "/restricted-Spin_";
    for(int coord: restricted_spin_coords)
    {
        filename += std::to_string(coord)+"_"; 
    }
    filename += "MIS-State.txt";
    #if __has_include(<filesystem>) // 如果系统支持C++17的<filesystem>库
        if (!fs::exists(dir)) { // 检查目标目录是否存在
            fs::create_directories(dir); // 如果目录不存在，则递归创建它及其所有父目录
        }
    ...
    outfile << "Index: " << restricted_index <<std::endl;
    outfile << "Restricted MIS: " << std::endl;
    if (p.total_nodes_for_bitpacking > 0) { // 如果晶格中有节点 (MIS状态与晶格节点一一对应)
        outfile << get_bit(restricted_mis_packed_array, 0); // 写入第一个节点的MIS归属状态 (0或1)
        for (long long i = 1; i < p.total_nodes_for_bitpacking; ++i) { // 遍历剩余的所有节点
            outfile << " " << get_bit(restricted_mis_packed_array, i); // 写入节点的MIS归属状态，用空格分隔
        }
    }
    outfile.close(); // 关闭文件流，确保所有数据写入磁盘
    std::cout << "Saved restricted MIS state to " << filename << std::endl; // 向标准输出流打印保存成功的信息和文件名    
}
...
MisResult calculate_mis_representation_packed( // 函数 calculate_mis_representation_packed：根据给定的晶格状态 (位打包)，计算其对应的最大独立集 (MIS) 及其位打包表示
    ...
    if (result.nU > 0 && result.nV > 0) { // 如果U和V部分都非空，可以运行Hopcroft-Karp算法
        ...
        // 根据最大匹配结果，使用Konig定理相关方法找到MIS的节点
...
void calculate_and_print_mis_statistics( // 函数 calculate_and_print_mis_statistics：计算并打印与MIS相关的各种统计数据
    const MisResult& mis_result, // 输入的MIS计算结果对象
    const Params& p, // 程序参数
    std::chrono::duration<double, std::milli> generation_duration, // 图生成及规则应用阶段的耗时
    std::chrono::duration<double, std::milli> mis_duration, // MIS计算阶段的耗时 (包括二分图构建和Hopcroft-Karp)
    std::string output_dir
) {
    long long num_nodes_in_active_graph = static_cast<long long>(mis_result.nU) + mis_result.nV; // 活跃图G (即晶格中值为1的节点构成的子图) 中的总节点数

    long long mis_nodes_in_U_partition = 0; // MIS中属于原始二分划分U部分的节点数量
    long long mis_nodes_in_V_partition = 0; // MIS中属于原始二分划分V部分的节点数量
    double order_parameter = 0.0; // 定义一个序参数，用于衡量MIS在U、V部分分布的不平衡性
    bool order_parameter_is_valid = (num_nodes_in_active_graph > 0); // 仅当活跃图非空时，序参数才有意义

    if (order_parameter_is_valid) { // 如果活跃图G非空
        for (int mis_node_global_idx : mis_result.mis_nodes_global_indices) { // 遍历MIS中的每一个节点 (全局索引)
            // 需要根据全局索引重新判断该MIS节点属于U还是V部分 (基于坐标和奇偶性)
            std::vector<int> coords = index_to_coords(mis_node_global_idx, p.L, p.W, p.d); // 获取节点坐标
            int coord_sum = 0; // 初始化坐标和
            for (int c : coords) coord_sum += c; // 计算坐标和

            if (coord_sum % 2 == 0) { // 如果坐标和为偶数 (属于图G的U划分)
                mis_nodes_in_U_partition++; // U部分的MIS节点计数增加
            } else { // 如果坐标和为奇数 (属于图G的V划分)
                mis_nodes_in_V_partition++; // V部分的MIS节点计数增加
            }
        }
        ...
        // 特殊情况处理：如果G的U部(或V部)为空，但MIS中却计入了U部(或V部)的节点 (理论上不应发生，除非定义特殊)
        // 若 mis_result.nU == 0 (G的U部为空)，则 mis_nodes_in_U_partition 也应为0。
        // 若此时 mis_nodes_in_U_partition > 0，将其比例设为1.0可能是一种对极端情况的约定处理。
        if (mis_result.nU == 0 && mis_nodes_in_U_partition > 0) prop_mis_in_U = 1.0; 
        if (mis_result.nV == 0 && mis_nodes_in_V_partition > 0) prop_mis_in_V = 1.0; 

        order_parameter = prop_mis_in_U - prop_mis_in_V; // 计算序参数：(U部MIS比例) - (V部MIS比例)
    }
    std::string outfile_name = output_dir + "/MIS_statistics.txt";
    std::ofstream output_file(outfile_name);
    if(!output_file.is_open())
    {
        std::cerr << "Can not open file " << outfile_name << std::endl;
    }
    output_file << "--- Statistics ---" << std::endl; // 输出统计信息部分的标题
    output_file << "  (d, L, W, rho, seed): (" << p.d << ", " << p.L << ", " << p.W << ", " << format_rho_string(p.rho) << ", " << p.seed << ")" << std::endl; // 输出当前模拟使用的参数

    // 计算最终晶格 (规则应用后) 中状态为1的节点的比例
    double proportion_ones_in_final_lattice = (p.total_nodes_for_bitpacking > 0) ? static_cast<double>(num_nodes_in_active_graph) / p.total_nodes_for_bitpacking : 0.0;
    output_file << "  Number of nodes in active graph G (lattice == 1): " << num_nodes_in_active_graph << std::endl; // 输出活跃图G中的节点总数
    output_file << "    Nodes in U partition of G: " << mis_result.nU << std::endl; // 输出G的U部分的节点数
    output_file << "    Nodes in V partition of G: " << mis_result.nV << std::endl; // 输出G的V部分的节点数
    output_file << "  Proportion of 1s in final lattice (after rules): " << std::fixed << std::setprecision(4) << proportion_ones_in_final_lattice << std::endl; // 输出最终晶格中1的比例 (保留4位小数)

    long long sum_mis = mis_result.mis_nodes_global_indices.size(); // MIS的大小 (即MIS中的节点总数)
    output_file << "  Size of Maximum Matching (MCM): " << mis_result.mcm_size << std::endl; // 输出最大匹配 (MCM) 的大小
    output_file << "  Size of Maximum Independent Set (MIS): " << sum_mis << std::endl; // 输出最大独立集 (MIS) 的大小

    if (num_nodes_in_active_graph > 0) { // 如果活跃图G非空
        double mis_ratio_to_G_size = static_cast<double>(sum_mis) / num_nodes_in_active_graph; // 计算MIS大小与G中节点总数的比率
        output_file << "  sum(MIS) / |V(G)|: " << std::fixed << std::setprecision(4) << mis_ratio_to_G_size << std::endl; // 输出该比率
    } else { // 如果活跃图G为空
         output_file << "  sum(MIS) / |V(G)|: N/A (G is empty)" << std::endl; // 提示该比率不适用
    }

    if (order_parameter_is_valid) { // 如果序参数有效
        output_file << "  MIS nodes count in U part (of G): " << mis_nodes_in_U_partition << std::endl; // 输出MIS在G的U部分的节点数
        output_file << "  MIS nodes count in V part (of G): " << mis_nodes_in_V_partition << std::endl; // 输出MIS在G的V部分的节点数
        output_file << "  Order Parameter ((MIS_U_nodes/|U_part_of_G|) - (MIS_V_nodes/|V_part_of_G|)): " << std::fixed << std::setprecision(4) << order_parameter << std::endl; // 输出计算得到的序参数
    } else { // 如果序参数无效 (因为G为空)
        output_file << "  Order Parameter: N/A (G is empty)" << std::endl; // 提示序参数不适用
    }

    output_file << "  Graph generation & rule application time: " << std::fixed << std::setprecision(2) << generation_duration.count() << " ms" << std::endl; // 输出图生成和规则应用的总耗时 (毫秒，保留2位小数)
    output_file << "  MIS finding time (incl. G construction, Hopcroft-Karp): " << std::fixed << std::setprecision(2) << mis_duration.count() << " ms" << std::endl; // 输出MIS计算的总耗时 (毫秒，保留2位小数)

    output_file.close();
} // calculate_and_print_mis_statistics 函数定义结束
...
std::vector<uint64_t> get_restricted_lattice(const std::vector<uint64_t>& lattice_packed, const Params& p,  long long index, bool Is_index_in_MIS)
{
    std::vector<uint64_t> restricted_lattice_packed(lattice_packed);
    set_bit(restricted_lattice_packed, index, 0);//为保证节点满足约束，将其设置为非活跃节点
    if(!Is_index_in_MIS)
    //不处于MIS中的节点，约束要求其必须处于MIS中，将该节点与其邻居均置为非活跃节点
    {
       std::vector<int> neighbors =  get_neighbors(index, p);
       if(!neighbors.empty())
       {
        for(int neighbor_index: neighbors)
        {
            set_bit(restricted_lattice_packed, neighbor_index, 0);
        }
       }
    }

    return restricted_lattice_packed;
}

std::vector<long long> get_active_spin_list(const std::vector<uint64_t>& lattice_packed, const Params& p)
{
    std::vector<long long> active_spin_list; 
    active_spin_list.reserve(static_cast<int>(p.total_nodes_for_bitpacking * p.rho) + 10);
    for(long long index =0; index < p.total_nodes_for_bitpacking; index++)
    {
        if(get_bit(lattice_packed,index) == 1)
        {
            active_spin_list.emplace_back(index);
        }
    }

    return active_spin_list;
...
void run_sample(const Params& base_params, unsigned int sample_seed, int sample_id, std::string base_dir )
{
    Params p = base_params;
    p.seed = sample_seed;
    std::string output_dir = base_dir +"/sample_" + std::to_string(sample_id);
    std::cout << "Processing sample " << sample_id << " with random seed: " << p.seed << " Output dir: "
    << output_dir <<std::endl;
    ...
    save_state_to_file_packed(lattice_packed, Max_size_connected_component_packed, mis_result.mis_lattice_representation_packed, p, output_dir);

    calculate_and_print_mis_statistics(mis_result, p, generation_duration, mis_duration,output_dir); // 计算并打印与MIS相关的各种统计数据及耗时信息
    std::string restricted_filename = output_dir+"/restricted_states.txt";

    std::ofstream output_restricted_file(restricted_filename);
    if(!output_restricted_file.is_open())
    {
        std::cerr<<" Unable to open file: "<< restricted_filename << std::endl;
        return;
    }

    output_restricted_file << "Restricted index: Restricted MIS state packed" << std::endl;
    output_restricted_file.flush();
    auto active_spin_list = get_active_spin_list(lattice_packed, p);
    std::vector<int> energy_change_list; energy_change_list.reserve(active_spin_list.size());
    std::vector<int> Num_change_list; Num_change_list.reserve(active_spin_list.size());
    int Max_energy_shift = 0; int Max_num_change = 0;
   
    //针对每个活跃节点进行翻转操作
    for(int spin_index =0; spin_index< active_spin_list.size(); spin_index++ )
    {
        long long index = active_spin_list[spin_index];

        bool Is_index_in_MIS = get_bit(mis_result.mis_lattice_representation_packed,index) == 1;

        std::vector<uint64_t> restricted_lattice = lattice_packed; // 创建新副本
    
        // 在副本上应用约束
        set_bit(restricted_lattice, index, 0); // 将当前节点设为非活跃
    
        if(!Is_index_in_MIS) {
            // 如果节点不在MIS中，约束要求其邻居也必须为非活跃
            std::vector<int> neighbors = get_neighbors(index, p);
            if(!neighbors.empty()) {
                for(int neighbor_index: neighbors) {
                    set_bit(restricted_lattice, neighbor_index, 0);
                }
            }
        }

        {
            MisResult restricted_result = calculate_mis_representation_packed(restricted_lattice, p);

            if (!Is_index_in_MIS) //对于不在初始MIS中的节点，约束要求其必须处于MIS中。在将该节点与其邻居挖空后的晶格上计算MIS后，将该节点补充至MIS集合中
            {
                set_bit(restricted_result.mis_lattice_representation_packed, index, 1);
            }

            { // 保存部分
                output_restricted_file<< index <<": ";
                output_restricted_file.flush();
                int energy_shift = 0; int change_num = 0;
                for(int idx_temp = 0; idx_temp < active_spin_list.size(); idx_temp++ )
                {
                    long long i_temp = active_spin_list[idx_temp];
                    int mis_state = get_bit(mis_result.mis_lattice_representation_packed, i_temp);
                    int mis_state_restricted = get_bit(restricted_result.mis_lattice_representation_packed, i_temp);
                    if( mis_state != mis_state_restricted)
                    {
                        output_restricted_file << i_temp << ", " << mis_state_restricted << "; ";
                        change_num ++; energy_shift += 1-2*mis_state_restricted;
                    }
                }
                output_restricted_file << std::endl;
                if(energy_shift > Max_energy_shift){Max_energy_shift = energy_shift;}
                if(change_num > Max_num_change){Max_num_change = change_num;}
                output_restricted_file.flush();
                energy_change_list.emplace_back(energy_shift); Num_change_list.emplace_back(change_num);
            }
        }
    }

    output_restricted_file.close();



    std::string stats_restricted_filename = output_dir+"/restricted_stats.txt";
    std::string result_restricted_filename = output_dir + "/restricted_result.txt";
    std::vector<int> energy_stats(Max_energy_shift + 1,0); std::vector<int>change_stats(Max_num_change + 1 ,0);
    std::vector<int> energy_stats_LCC(Max_energy_shift + 1,0); std::vector<int>change_stats_LCC(Max_num_change + 1 ,0);
    std::ofstream result_file(result_restricted_filename);
   
    if(!result_file.is_open())
    {
        std::cerr<< "Can not open file" << result_restricted_filename << std::endl;
        return;
    }
    result_file << "index, occupation state in ground state, change number, change energy, Is in largest connected component" << std::endl;
    for(int spin_idx = 0; spin_idx < energy_change_list.size(); spin_idx++)
    {
        int vertex_index = active_spin_list[spin_idx];
        int energy_change = energy_change_list[spin_idx];
        int change_num = Num_change_list[spin_idx];
        bool Is_in_MAX_connected_component = get_bit(Max_size_connected_component_packed, spin_idx) == 1;
        energy_stats[energy_change] += 1;
        change_stats[change_num] += 1;
        if(Is_in_MAX_connected_component){energy_stats_LCC[energy_change] += 1; change_stats_LCC[change_num] += 1;}
        result_file<< vertex_index << ", "
                   << get_bit(mis_result.mis_lattice_representation_packed,vertex_index) << ", "
                   << change_num << ", "
                   << energy_change << ", "
                   << Is_in_MAX_connected_component <<std::endl; 
    } 
    result_file.close();

    std::ofstream stats_file(stats_restricted_filename);
    if(!stats_file.is_open())
    {
        std::cerr<< "Can not open file" << stats_restricted_filename << std::endl;
        return;
    }
    stats_file << "energy shift value (Whole lattice):  " <<std::endl;
    for(int i = 0; i <energy_stats.size();i++)
    {
        if(!energy_stats[i]){continue;}
        stats_file << i <<  ": " << energy_stats[i] << std::endl;
    }

    stats_file << "Change vertex number (Whole lattice): " << std::endl;
    for(int i = 0; i < change_stats.size(); i++)
    {
        if(!change_stats[i]){continue;}
        stats_file << i << ": " << change_stats[i] << std::endl;
    }
    stats_file << "energy shift value (On largest connected component):  " <<std::endl;
    for(int i = 0; i <energy_stats_LCC.size();i++)
    {
        if(!energy_stats_LCC[i]){continue;}
        stats_file << i <<  ": " << energy_stats_LCC[i] << std::endl;
    }

    stats_file << "Change vertex number (On largest connected component): " << std::endl;
    for(int i = 0; i < change_stats_LCC.size(); i++)
    {
        if(!change_stats_LCC[i]){continue;}
        stats_file << i << ": " << change_stats_LCC[i] << std::endl;
    }
    stats_file.close();
}
...
void worker_thread(ThreadSafeQueue<SampleTask>& task_queue, const Params& base_params, 
    ...
    while (task_queue.try_pop(task)) {
        run_sample(base_params, task.seed, task.sample_id, task.output_dir);
...
//g++ -O3 -march=native -pthread -o MIS_parallel.exe MIS_sample_parallel.cpp
//MIS_parallel.exe 2 40 40 0.16 114514 1 100 B 10 

int main(int argc, char* argv[]) { // 主函数：程序入口点
    Params p; // 创建一个Params结构体实例，用于存储所有模拟参数

    // 设置参数的默认值
    p.d = 2; // 默认维度为2
    p.L = 20 ; // 默认第一个维度 (长度) 为200
    p.W = p.L; // 默认其他维度 (宽度) 与长度相同
    p.rho = 0.1; // 默认rho参数为0.14
    p.seed = 114514; // 默认随机数种子
    p.periodic_boundary = true; // 默认不使用周期性边界条件
    p.num_samples = 100; //默认样本数目
    p. boundary_prefer = 'B'; //"A" 或 "B"
    unsigned int Max_thread_number = 5;
    std::string dir = "MIS-graph/mis-data";
    // 从命令行参数解析并覆盖默认参数值
    if (argc > 1) p.d = std::stoi(argv[1]); // 如果提供了至少1个额外参数，将其视为维度d
    if (argc > 2) p.L = std::stoi(argv[2]); // 第2个额外参数视为长度L
    if (argc > 3) p.W = std::stoi(argv[3]); // 第3个额外参数视为宽度W
    if (argc > 4) p.rho = std::stod(argv[4]); // 第4个额外参数视为rho值 (double类型)
    if (argc > 5) { // 第5个额外参数视为随机数种子
        try {
            long long temp_seed = std::stoll(argv[5]); // 尝试将参数字符串转换为long long类型
            if (temp_seed == 0) { // 如果提供的种子是0，则特殊处理为使用随机设备生成种子
                 p.seed = 0;
            } else if (temp_seed < 0 || temp_seed > std::numeric_limits<unsigned int>::max()) { // 检查种子是否在unsigned int有效范围内
                std::cerr << "Warning: Provided seed " << temp_seed << " is out of range for unsigned int. Using random_device." << std::endl; // 超出范围则警告
                p.seed = 0; // 并设置为0，后续会用随机设备生成
            } else {
                p.seed = static_cast<unsigned int>(temp_seed); // 合法范围内，转换为unsigned int类型
            }
        } catch (const std::out_of_range& oor) { // 捕获stoll转换时可能发生的超出范围异常
            std::cerr << "Warning: Seed value " << argv[5] << " is too large. Using random_device." << std::endl; // 警告
            p.seed = 0; // 使用随机设备
        } catch (const std::invalid_argument& ia) { // 捕获stoll转换时可能发生的无效参数异常
            std::cerr << "Warning: Invalid seed value '" << argv[5] << "'. Using random_device." << std::endl; // 警告
            p.seed = 0; // 使用随机设备
        }
    }
    if (argc > 6) p.periodic_boundary = (std::stoi(argv[6]) == 1); // 第6个参数：1表示使用周期性边界，0表示不使用
    if (argc > 7) p.num_samples = std::stoi(argv[7]); // 第7个参数： 生成样本数目
    if (argc > 8) { // 第8个参数： 边界偏好参数（非周期性边界）
        std::string input_param = argv[8];
        std::string upper_input = input_param;
        // 转换为大写以便不区分大小写比较
        std::transform(upper_input.begin(), upper_input.end(), upper_input.begin(), [](unsigned char c){ return std::toupper(c); });
    
        if ( upper_input == "A" || upper_input == "B") {
            p.boundary_prefer = upper_input[0];  // 取大写形式
        } else {
            std::cerr << "Warning: Invalid boundary preference '" << argv[8]   << "'. Using default value " << p.boundary_prefer << std::endl;
        // 这里可以使用默认值，比如：boundary_prefer = 'A'; 或其他默认值
        }
    }
    if (argc > 9) {// 第9个参数： 最大线程数（不超过16）
        int number_thread = std::stoi(argv[9]);
        if(number_thread > 0)
        {
        ...
        {
            std::cerr << "Invalid thread number input, using defalt thread number " << Max_thread_number << std::endl;
        }
    }
    if (argc > 10) //第10个参数： 主输出目录
    {
        std::string input_dir = argv[10];
        bool Is_dir_exist = false;
        #if __has_include(<filesystem>) // 如果系统支持C++17的<filesystem>库
            if (fs::exists(input_dir)) { // 检查目标目录是否存在
                Is_dir_exist = true;} 
        #else // 如果系统不支持<filesystem>，使用兼容的备用方案
            struct stat st; // 定义文件状态结构体
            if (stat(input_dir.c_str(), &st) != -1) { // 尝试获取目录状态
                Is_dir_exist = true;
            }
        #endif // __has_include(<filesystem>) 结束
        if(Is_dir_exist)
        ...
        {
            std::cerr << "Can not find the input directory " << input_dir << " , using defalt main output directory " << dir << std::endl;
        }
    }

    long long total_nodes_val = 1; // 初始化总节点数。对于d=0的情况，默认为1。
    try { // 使用try-catch块来处理计算总节点数时可能发生的错误 (如溢出)
        if (p.d < 0 || p.L < 0 || p.W < 0) throw std::runtime_error("d, L and W must be non-negative"); // 检查维度和尺寸是否非负
        if (p.d == 0) { // 0维情况
            total_nodes_val = 1; // 0维空间只有1个节点
        } else if (p.d == 1) { // 1维情况
            total_nodes_val = p.L; // 总节点数等于长度L
        } else { // d > 1 维情况 (L * W^(d-1))
            total_nodes_val = p.L; // 从L开始
            for(int i = 1; i < p.d; ++i) { // 乘以 (d-1) 次W
                // 使用128位整数进行中间计算，以防止long long溢出
                unsigned __int128 temp_total_nodes = static_cast<unsigned __int128>(total_nodes_val) * p.W;
                if (temp_total_nodes > std::numeric_limits<long long>::max()) { // 如果计算结果超过long long的最大表示范围
                     throw std::overflow_error("L × W^(d-1) too large for long long"); // 抛出溢出错误
                }
                total_nodes_val = static_cast<long long>(temp_total_nodes); // 将结果转换回long long
            }
        }
        // 对计算出的 total_nodes_val 进行合理性检查，确保其非负，除非是特定L/W=0的有效情况
        // 这个复杂的条件是为了区分有效的0节点情况 (如L=0) 和因参数错误导致的非正节点数
        if (total_nodes_val <= 0 && !(p.L==0 && p.d==1) && !(p.L==0 && p.d>=2 && p.W>0) && !(p.W==0 && p.d>=2) && !(p.d==0 && total_nodes_val!=1) ) { 
             if (total_nodes_val == 0 && ( (p.L==0 && (p.d==1 || p.d>=2)) || (p.d>=2 && p.W==0) ) ) { /* 这是预期的0节点情况，例如L=0或(d>=2且W=0) */ }
             else if (p.d == 0 && total_nodes_val == 1) { /* d=0时，1个节点是正确的 */ }
             else { // 其他导致 total_nodes_val <= 0 的意外情况
                  throw std::overflow_error("Invalid L/W/d combination results in non-positive total_nodes: " + std::to_string(total_nodes_val));
             }
        }
        if (total_nodes_val < 0) total_nodes_val = 0; // 作为最终保障，如果 total_nodes_val 为负，则强制为0 (更佳做法是早些抛出错误)

    } catch (const std::overflow_error& e) { // 捕获计算总节点数时的溢出错误
        std::cerr << "Error: L × W^(d-1) resulted in overflow. L=" << p.L << ", W=" << p.W << ", d=" << p.d << ". " << e.what() << std::endl;
        return 1; // 程序异常退出，返回1
    } catch (const std::runtime_error& e) { // 捕获其他运行时错误 (例如 d,L,W 为负)
        std::cerr << "Error: " << e.what() << std::endl;
        return 1; // 程序异常退出，返回1
    }
    p.total_nodes_for_bitpacking = total_nodes_val; // 将计算得到的总节点数存储到参数结构中

    std::string Out_param_boundary_condition = p.periodic_boundary == 1 ? "Periodic_boundary" : (p.boundary_prefer == 'A'? "Preference boundary: A":"Preference boundary: B");
    // 打印最终确认的模拟参数
    std::cout << "Parameters: d=" << p.d << ", L=" << p.L << ", W=" << p.W << ", rho=" << p.rho
              << ", seed=" << p.seed << ", " << Out_param_boundary_condition
              << ", total_nodes=" << p.total_nodes_for_bitpacking;
    if(!p.periodic_boundary) //只在不使用周期性边界条件时考虑约束性外部边界条件
    {
        std::cout<<", boundary_condition="<< p.boundary_prefer;
    }
    std::cout << ", sample number=" << p.num_samples << ", max thread number=" << Max_thread_number << ", main output directory: " << dir << std::endl;
    ...
    p.L_multipliers.assign(p.d, 0LL); // 初始化维度乘数向量，大小为d，所有元素为0 (long long 0)
    if (p.d > 0) { // 仅当维度大于0时计算乘数
        if (p.d == 1) { // 一维情况
            if (p.L > 0) p.L_multipliers[0] = 1LL; // 第0维 (也是唯一维) 的乘数为1
        } else { // d > 1 维情况 (维度顺序: L, W, W, ...)
            // 最后一个维度 (索引 d-1) 的乘数是1 (如果该维度大小W > 0)
            if (p.W > 0) p.L_multipliers[p.d - 1] = 1LL; else p.L_multipliers[p.d-1] = 0LL;

            // 从倒数第二个维度 (索引 d-2) 向前计算到第二个维度 (索引 1)
            // L_multipliers[k] = L_multipliers[k+1] * W
            for (int k = p.d - 2; k >= 1; --k) { 
                if (p.W == 0)
                { p.L_multipliers[k] = 0LL; continue; } // 如果W=0，则更高维的乘数也为0

                unsigned __int128 temp_mult;
                temp_mult = static_cast<unsigned __int128>(p.L_multipliers[k + 1] * p.W); // 使用128位整溢出
                 
                if (temp_mult > std::numeric_limits<long long>::max()){ // 检查是否溢出long long
                    std::cerr << "Warning: L_multiplier overflow for W=" << p.W << ", d=" << p.d << ". Results may be incorrect." << std::endl;
                    p.L_multipliers[k] = std::numeric_limits<long long>::max(); // 溢出则设为最大值
                } else {
                   p.L_multipliers[k] = static_cast<long long>(temp_mult); // 存储计算得到的乘数
                }
            }
            
            // 计算第一个维度 (索引 0, 对应L) 的乘数
            // L_multipliers[0] = L_multipliers[1] * W (如果 d >= 2)
            if (p.d >= 2 && p.W > 0 && p.L_multipliers[1] > 0) { // 确保维度至少为2, W>0, 且维度1的乘数有效
                 unsigned __int128 temp_mult = static_cast<unsigned __int128>(p.L_multipliers[1]) * p.W;
                 if (temp_mult > std::numeric_limits<long long>::max()){ // 检查溢出
                    ...
                    p.L_multipliers[0] = std::numeric_limits<long long>::max(); // 溢出则设为最大值
                } else {
                   p.L_multipliers[0] = static_cast<long long>(temp_mult); // 存储乘数
                }
            } else if (p.d >= 2) { // 如果 d>=2 但 W=0 或 L_multipliers[1]=0 (导致无法正确计算)
                p.L_multipliers[0] = 0LL; // 第一个维度的乘数设为0
            }
            // 如果 d=1, L_multipliers[0] 已在前面处理。
        }
    }
    
    // 如果总节点数非常大，并且不是细长或扁平的形状，则发出警告
    if (p.total_nodes_for_bitpacking > 20000000 && p.L > 10 && p.W > 10 && p.d > 1) {
        std::cerr << "Warning: total_nodes = " << p.total_nodes_for_bitpacking << " is very large. Computation might be slow or memory intensive." << std::endl;
    }
    // 如果总节点数为0 (例如L=0)，则提前结束模拟
    if (p.total_nodes_for_bitpacking == 0) {
        std::cout << "Total nodes is 0. Exiting simulation early." << std::endl;
    ...
    dir = dir + "/d="+ std::to_string(p.d) + // 构建完整的文件名，包含维度、尺寸、rho和种子等参数信息
                           "-L=" + std::to_string(p.L) + "-W=" + std::to_string(p.W) + boundary_str  + 
                           "-rho=" + rho_str + "-main_seed=" + std::to_string(p.seed);
    ...
    if (num_threads == 0) {
        num_threads = Max_thread_number; // 如果无法获取硬件并发数，默认使用4个线程
    }
    num_threads = std::min(num_threads,Max_thread_number);
    std::cout << "Using " << num_threads << " threads for processing " << p.num_samples << " samples." << std::endl;
    // 创建任务队列和完成计数器
    ...
    for (int sample_id = 0; sample_id < p.num_samples; ++sample_id) {
        ...
        task.output_dir = dir; // 使用主目录，每个样本会在其中创建子目录
    ...
    std::cout << "All " << p.num_samples << " samples processed successfully!" << std::endl;
    return 0; // 主函数执行完毕，程序正常结束，返回0
} 
